...
*/
	...
	public function print_preview() {
		if(permissionChecker('student_view') || (($this->session->userdata('usertypeID') == 3) && permissionChecker('student') && ($this->session->userdata('loginuserID') == htmlentities((string) escapeString($this->uri->segment(3)))))) {
	...
	public function delete() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id && (int)$url) {
				...
				if(customCompute($this->data['student'])) {
					if(config_item('demo') == FALSE && ($this->data['student']->photo != 'default.png' && $this->data['student']->photo != 'defualt.png') && file_exists(FCPATH.'uploads/images/'.$this->data['student']->photo)) {
	...
	public function changeSection() {
		...
		if (!(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1))) {
			$retArray['message'] = $this->lang->line('student_section_update_error');
			echo json_encode($retArray);
			exit;
		}
		...
		if (!$classesID) {
			$retArray['message'] = $this->lang->line('student_select_class');
			echo json_encode($retArray);
			exit;
		}
		if (!$sectionID) {
			$retArray['message'] = $this->lang->line('student_section_required');
			echo json_encode($retArray);
			exit;
		}
		if (empty($studentIDs) || !is_array($studentIDs)) {
			$retArray['message'] = $this->lang->line('student_select_students');
			echo json_encode($retArray);
			exit;
		}

		$section = $this->section_m->general_get_single_section(['sectionID' => $sectionID, 'classesID' => $classesID]);
		if (!customCompute($section)) {
			$retArray['message'] = $this->lang->line('student_section_invalid');
			echo json_encode($retArray);
			exit;
		}
		...
		foreach ($studentIDs as $studentID) {
			...
			$this->student_m->update_student(['sectionID' => $sectionID], $studentID);
		...
		if ($updated > 0) {
			$retArray['status'] = TRUE;
			$retArray['message'] = $updated . ' ' . $this->lang->line('student_section_updated');
			$this->session->set_flashdata('success', $retArray['message']);
		} else {
			$retArray['message'] = $this->lang->line('student_no_students_selected');
		}
