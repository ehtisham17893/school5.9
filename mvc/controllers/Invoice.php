    ...
    */
    ...
    public function index1()
    {
        ...
        if($usertypeID == 3) {
            ...
            $this->data['maininvoices']         = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID,$classesID);
            $this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['maininvoices'], $schoolyearID);
            $this->data["subview"]              = "invoice/index";
            $this->load->view('_layout_main', $this->data);
        }
    ...
    public function edit()
    {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
            ...
            $maininvoiceID = htmlentities((string) escapeString($this->uri->segment(3)));
            if((int)$maininvoiceID !== 0) {
                $schoolyearID                = $this->session->userdata('defaultschoolyearID');
                ...
                if(customCompute($this->data['maininvoice'])) {
                    if($this->data['maininvoice']->maininvoicestatus == 0 || $this->data['maininvoice']->maininvoicestatus == 1) {
                        $this->data['classes']  = $this->classes_m->general_get_classes();
                        ...
                        $this->data['invoices'] = $this->invoice_m->get_order_by_invoice1(['maininvoiceID' => $maininvoiceID]);
    ...
    //function for bulk invoicing
    public function bulkInvoicing(){

        $i=0;
        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        $this->data['feetypes'] = pluck($this->feetypes_m->get_feetypes(), 'feetypes', 'feetypesID');
        $invoices_id = htmlentities(escapeString($this->uri->segment(3)));
        $invoicesArray=explode('-',$invoices_id);
        if(sizeof($invoicesArray)>0) {
            foreach ($invoicesArray as $invoiceSingle){
                $this->data['maininvoice']=null;
                $this->data['invoices']=null;
                $this->data['grandtotalandpayment']=null;
                $this->data['student']=null;
                $this->data['createuser']=null;

                $this->data['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_maininvoiceID($invoiceSingle, $schoolyearID);
                $this->data['invoices'] = $this->invoice_m->get_order_by_invoice1(array('maininvoiceID' => $invoiceSingle));
                ...
                $this->finalData[$i]['mainInvoice']=$this->data['maininvoice'];
                $this->finalData[$i]['invoices']=$this->data['invoices'];
                $this->finalData[$i]['grandtotalandpayment']=$this->data['grandtotalandpayment'];
                $this->finalData[$i]['student']=$this->data['student'];
                $this->finalData[$i]['createuser']=$this->data['createuser'];
                $i++;
        ...
        $this->data["finalData"]=$this->finalData;
        $this->data["subview"] = "invoice/viewall";
        $this->load->view('_layout_main', $this->data);
    }
    //end of function for bulk invoicing

    public function add_heads_to_invoices() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
            ...
            $this->data["subview"] = "invoice/add_heads_to_invoices";
            $this->load->view('_layout_main', $this->data);
        } else {
            $this->data["subview"] = "error";
            $this->load->view('_layout_main', $this->data);
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        if(((int)$studentID || $studentID == 0) && (int)($classesID)) {
                            ...
                            if(count($getstudents)) {
                                ...
                                $studentsCount = count($getstudents);

                                foreach ($getstudents as $key => $getstudent){
                                    $maininvoice_id = $this->maininvoice_m->get_maininvoiceID($getstudent->srstudentID, $schoolyearID);
                                    $this->maininvoice_m->update_by_ID(1,$maininvoice_id);
                                    //var_dump($maininvoice_id);exit();
                                    if ($maininvoice_id != 0){
            ...
            if((int)$id !== 0) {
                ...
                if(customCompute($getstudent)) {
                    ...
                    if(customCompute($this->data['maininvoice']) && ($this->data['maininvoice']->maininvoicestudentID == $getstudent->studentID)) {
                        $this->data['invoices'] = $this->invoice_m->get_order_by_invoice1(['maininvoiceID' => $id]);
                ...
                if($this->form_validation->run() == FALSE) {
                    ...
                    if($usertypeID == 3) {
                        ...
                        if((int)$id !== 0) {
                            ...
                            if(customCompute($fetchStudent)) {
                                $this->data['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_maininvoiceID($id, $schoolyearID);
                                if(customCompute($this->data['maininvoice']) && in_array($this->data['maininvoice']->maininvoicestudentID, $fetchStudent)) {
                                    $f = TRUE;
                                }
                            }
                        }
    ...
    protected function payment_rules( $invoices )
    {
        ...
        if($invoices && customCompute($invoices)) {
    ...
    public function unique_givenamount( $postValue )
    {
        ...
        } elseif ($this->_amountgivenstatus) {
            if($postValue != '') {
    ...
    public function unique_payment_method()
    {
        ...
        if(in_array(ucfirst((string) $this->input->post('payment_method')), $this->payment_methods($payment_methods))) {
            if(ucfirst((string) $this->input->post('payment_method')) === 'Select') {
                    ...
                    if($maininvoice->maininvoicestatus != 2) {
                        ...
                        if(customCompute($this->data['student'])) {
                            ...
                            if($usertypeID == 3) {
                                ...
                                $fetchStudent = pluck($getStudents, 'srstudentID', 'srstudentID');
                                if(customCompute($fetchStudent) && in_array($this->data['student']->srstudentID, $fetchStudent)) {
                                    $f = TRUE;
                                }
                            } else {
                                $f = TRUE;
    ...
    {
        if(permissionChecker('invoice_view')) {
            $globalpaymentID = htmlentities((string) escapeString($this->uri->segment(3)));
            $maininvoiceID   = htmlentities((string) escapeString($this->uri->segment(4)));
            $schoolyearID    = $this->session->userdata('defaultschoolyearID');
    ...
    public function unique_studentID()
    {
        ...
        if ((int)$id && $id > 0 && $this->input->post('studentID') == 0) {
    ...
    {
        $feetypeitems = json_decode((string) $this->input->post('feetypeitems'));
        $status       = [];
                        ...
                        if(((int)$studentID || $studentID == 0) && (int)($classesID)) {
                            ...
                            if(customCompute($getstudents)) {
                                ...
                                if($this->input->post('statusID') !== '0') {
                                    if ((float)$this->input->post('totalsubtotal') == (float)0) {
                                        $paymentStatus = 2;
                                    } elseif ((float)$this->input->post('totalpaidamount') > (float)0) {
                                        $paymentStatus = (float)$this->input->post('totalsubtotal') === (float)$this->input->post('totalpaidamount') ? 2 : 1;
                                    }
                                }
                                ...
                                foreach($getstudents as $key => $getstudent) {
                                    $invoiceMainArray[] = [
                                        ...
                                        'maininvoiceusertypeID'   => $this->session->userdata('usertypeID'),
                                        'maininvoiceuname'        => $this->session->userdata('name'),
                                        'maininvoicedate'         => date("Y-m-d", strtotime((string) $this->input->post("date"))),
                                        'maininvoicecreate_date'  => date('Y-m-d'),
                                        'maininvoiceday'          => date('d'),
                                        'maininvoicemonth'        => date('m'),
                                        'maininvoiceyear'         => date('Y'),
                                        ...
                                        for($i = $firstID; $i <= $lastID; $i++) {
                                            if(customCompute($feetypeitems)) {
                                                foreach($feetypeitems as $feetypeitem) {
                                                    ...
                                                    $paymentHistoryArray[] = [
                                                        'paymenttype'   => ucfirst((string) $this->input->post('payment_method')),
                                                        'paymentamount' => $feetypeitem->paidamount
                                                    ];
                                                }
                                            }
                                ...
                                if ($paymentInserStatus && $invoiceSubtotalStatus && customCompute($invoiceArray)) {
                            ...
                            if((int)$studentID && (int)$classesID) {
                                ...
                                if(customCompute($getstudent)) {
                                    ...
                                    if($this->input->post('statusID') !== '0') {
                                        if ((float)$this->input->post('totalsubtotal') == (float)0) {
                                            $paymentStatus = 2;
                                        } elseif ($this->input->post('totalpaidamount') > 0) {
                                            $paymentStatus = (float)$this->input->post('totalsubtotal') === (float)$this->input->post('totalpaidamount') ? 2 : 1;
                                        }
                                    }
                                    ...
                                    if ($paymentInserStatus !== 0 && customCompute($invoiceArray)) {
            ...
            if(!empty($maininvoiceID) && (int)$maininvoiceID && $maininvoiceID > 0) {
                ...
                if(customCompute($paymentlists)) {
                    ...
                    foreach($paymentlists as $key => $paymentlist) {
                        ...
                        echo date('d M Y', strtotime((string) $paymentlist['date']));
                        echo '</td>';
                        ...
                        if((($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || $this->session->userdata('usertypeID') == 5) && (($this->lang->line('Cash') == $paymentlist['paymenttype']) || ($this->lang->line('Cheque') == $paymentlist['paymenttype']) || ('Cash' == $paymentlist['paymenttype']) || ('Cheque' == $paymentlist['paymenttype'])) && permissionChecker('invoice_delete')) {
    ...
    {
        if(isset($this->payment_gateway_array[htmlentities((string) escapeString($this->uri->segment(3)))])) {
            $this->payment_gateway->gateway(htmlentities((string) escapeString($this->uri->segment(3))))->weaver();
        }
    }
    ...
    public function generate()
    {
        ...
        $this->data["subview"] = "invoice/generate";
        $this->load->view('_layout_main', $this->data);
    }

    public function getReferenceFeetypes()
    {
        $classesID = $this->input->post('classesID');
        $schoolyearID = $this->session->userdata('defaultschoolyearID');

        if ((int)$classesID) {
            $this->db->select('DISTINCT(invoice.feetypeID), feetypes.feetypes');
            ...
            $this->db->order_by('invoice.feetypeID', 'DESC');
            $query = $this->db->get();
            $feetypes = $query->result();

            $options = '<option value="0">' . $this->lang->line('select_reference_feetype') . '</option>';
            if (customCompute($feetypes)) {
                foreach ($feetypes as $feetype) {
                    $options .= '<option value="' . $feetype->feetypeID . '">' . $feetype->feetypes . '</option>';
                }
            }
            echo $options;
        } else {
            echo '<option value="0">' . $this->lang->line('select_reference_feetype') . '</option>';
        }
    }
    ...
    public function generateInvoices()
    {
        ...
        $classesID = $this->input->post('classesID');
        $referenceFeetypeID = $this->input->post('referenceFeetypeID');
        $targetFeetypeID = $this->input->post('targetFeetypeID');
        $dueDate = $this->input->post('dueDate');
        $schoolyearID = $this->session->userdata('defaultschoolyearID');

        $errors = [];
        if (!(int)$classesID) {
            $errors[] = $this->lang->line('class_required');
        }
        if (!(int)$referenceFeetypeID) {
            $errors[] = $this->lang->line('reference_feetype_required');
        }
        if (!(int)$targetFeetypeID) {
            $errors[] = $this->lang->line('target_feetype_required');
        }
        if (empty($dueDate)) {
            $errors[] = $this->lang->line('due_date_required');
        }
        if ((int)$referenceFeetypeID === (int)$targetFeetypeID && (int)$referenceFeetypeID > 0) {
            $errors[] = $this->lang->line('same_feetype_error');
        }

        if (!empty($errors)) {
            $retArray['message'] = implode(', ', $errors);
            echo json_encode($retArray);
            exit;
        }

        $referenceInvoices = $this->invoice_m->get_order_by_invoice([
            'classesID' => $classesID,
            'feetypeID' => $referenceFeetypeID,
            'schoolyearID' => $schoolyearID,
            'deleted_at' => 1
        ]);

        if (!customCompute($referenceInvoices)) {
            $retArray['message'] = $this->lang->line('no_reference_data');
            echo json_encode($retArray);
            exit;
        }

        $targetFeetype = $this->feetypes_m->get_single_feetypes(['feetypesID' => $targetFeetypeID]);
        $targetFeetypeName = $targetFeetype ? $targetFeetype->feetypes : '';
        ...
        $skipped = 0;
        $invoiceArrayBatch = [];
        $skippedMaininvoiceIDs = [];

        $dueTs = strtotime($dueDateFormatted);
        $dueDay = date('d', $dueTs);
        $dueMonth = date('m', $dueTs);
        $dueYear = date('Y', $dueTs);

        foreach ($referenceInvoices as $refInvoice) {
            $existingInvoice = $this->invoice_m->get_single_invoice([
                'classesID' => $refInvoice->classesID,
                'studentID' => $refInvoice->studentID,
            ...
            if ($existingInvoice) {
                ...
                $skipMid = (int)$refInvoice->maininvoiceID;
                if ($skipMid > 0) {
                    $skippedMaininvoiceIDs[$skipMid] = true;
                }
                continue;
            }
            ...
            $invoiceArrayBatch[] = [
                ...
                'studentID' => $refInvoice->studentID,
                'feetypeID' => $targetFeetypeID,
                'feetype' => $targetFeetypeName,
                'amount' => $refInvoice->amount,
                'discount' => $refInvoice->discount,
                'paidstatus' => 0,
                'userID' => $this->session->userdata('loginuserID'),
                'usertypeID' => $this->session->userdata('usertypeID'),
                'uname' => $this->session->userdata('name'),
                ...
                'year' => $dueYear,
                'deleted_at' => 1,
                'maininvoiceID' => $refInvoice->maininvoiceID
            ];
            $created++;
        }

        if (customCompute($invoiceArrayBatch)) {
            $this->invoice_m->insert_batch_invoice($invoiceArrayBatch);

            $maininvoiceIDsToSync = [];
            foreach ($invoiceArrayBatch as $row) {
                $mid = isset($row['maininvoiceID']) ? (int)$row['maininvoiceID'] : 0;
                if ($mid > 0) {
                    $maininvoiceIDsToSync[$mid] = true;
                }
            }
            foreach (array_keys($maininvoiceIDsToSync) as $maininvoiceID) {
                $this->maininvoice_m->update_maininvoice([
                    'maininvoicestatus' => 0,
                    'maininvoicedate' => $dueDateFormatted,
                ], $maininvoiceID);
            }
        }

        foreach (array_keys($skippedMaininvoiceIDs) as $maininvoiceID) {
            $this->maininvoice_m->update_maininvoice([
                'maininvoicedate' => $dueDateFormatted,
            ], $maininvoiceID);
        }
        ...
        $retArray['message'] = $created . ' ' . $this->lang->line('invoices_created') . ', ' . $skipped . ' ' . $this->lang->line('invoices_skipped');
    ...
    public function invoicePayments()
    {
        ...
        $this->data["subview"] = "invoice/invoice_payments";
        $this->load->view('_layout_main', $this->data);
    }

    public function getInvoicePaymentSections()
    {
        $classesID = $this->input->post('classesID');
        $options = '<option value="0">' . $this->lang->line('invoice_select_section') . '</option>';

        if ((int)$classesID) {
            $sections = $this->section_m->general_get_order_by_section(['classesID' => $classesID]);
            if (customCompute($sections)) {
                foreach ($sections as $section) {
                    $options .= '<option value="' . $section->sectionID . '">' . $section->section . '</option>';
                }
            }
        }
        echo $options;
    ...
    public function getInvoicePaymentGrid()
    {
        header('Content-Type: application/json; charset=utf-8');
        $retArray = ['status' => FALSE, 'students' => [], 'feetypes' => [], 'grid' => [], 'payment_gateway' => [], 'error' => ''];
        ...
        try {
            ...
            if (!(int)$classesID) {
                $retArray['error'] = 'Class is required';
                echo json_encode($retArray);
                exit;
            }

            // Same student set as Invoice > index1 (class): maininvoice + studentrelation + active students.
            // Avoids missing rows when studentrelation section/class and dropdown section do not match.
            $maininvoiceRows = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID, $classesID);
            if (!customCompute($maininvoiceRows)) {
                $retArray['error'] = 'No students found';
                echo json_encode($retArray);
                exit;
            }

            $studentsById = [];
            foreach ($maininvoiceRows as $m) {
                if ((int)$sectionID && (int)$m->srsectionID !== (int)$sectionID) {
                    continue;
                }
                $sid = (int)$m->maininvoicestudentID;
                if ($sid && !isset($studentsById[$sid])) {
                    $studentsById[$sid] = $m;
                }
            }
            $students = array_values($studentsById);
            if (!customCompute($students)) {
                $retArray['error'] = 'No students found for this class and section';
                echo json_encode($retArray);
                exit;
            }

            $studentIDs = array_keys($studentsById);
        ...
        $this->db->order_by('invoice.feetypeID', 'ASC');
        $feetypesQuery = $this->db->get();
        $feetypes = $feetypesQuery->result();
        ...
        if (!customCompute($feetypes)) {
            ...
            $retArray['grid'] = [];
            $retArray['status'] = TRUE;
            $retArray['payment_gateway'] = $this->payment_methods($this->payment_gateway_m->get_order_by_payment_gateway(['status' => 1]));
            echo json_encode($retArray);
            exit;
        }
        ...
        $weaverByInvoice = [];
        $weaverItems = $this->weaverandfine_m->get_order_by_weaverandfine(['schoolyearID' => $schoolyearID]);
        if (customCompute($weaverItems)) {
            foreach ($weaverItems as $wf) {
                if (!isset($weaverByInvoice[$wf->invoiceID])) {
                    $weaverByInvoice[$wf->invoiceID] = ['weaver' => 0, 'fine' => 0];
                }
                $weaverByInvoice[$wf->invoiceID]['weaver'] += (float)$wf->weaver;
                $weaverByInvoice[$wf->invoiceID]['fine'] += (float)$wf->fine;
            }
        }

        $invoiceByStudentFeetype = [];
        foreach ($invoices as $inv) {
            if (!in_array($inv->studentID, $studentIDs)) {
                continue;
            }
            $key = $inv->studentID . '_' . $inv->feetypeID;
            $amount = $inv->amount;
            if ($inv->discount > 0) {
                $amount = $amount - (($amount / 100) * $inv->discount);
            }
            $paymentSum = $this->payment_m->get_payment_by_sum($inv->invoiceID);
            $paidTotal = $paymentSum && $paymentSum->paymentamount ? (float)$paymentSum->paymentamount : 0;
            $weaverTotal = isset($weaverByInvoice[$inv->invoiceID]) ? $weaverByInvoice[$inv->invoiceID]['weaver'] : 0;
            $due = $amount - $paidTotal - $weaverTotal;

            if ($due >= 0 && (!isset($invoiceByStudentFeetype[$key]) || $due > $invoiceByStudentFeetype[$key]['due'])) {
                $invoiceByStudentFeetype[$key] = [
        ...
        usort($students, function ($a, $b) {
            return strcasecmp($a->srname ?? '', $b->srname ?? '');
        });

        $grid = [];
        foreach ($students as $student) {
            $sid = (int)($student->maininvoicestudentID ?? $student->srstudentID);
            $row = ['studentID' => $sid, 'name' => $student->srname ?? '', 'roll' => $student->srroll ?? '', 'cells' => []];
            foreach ($feetypes as $ft) {
                $key = $sid . '_' . $ft->feetypeID;
                $cell = ['invoiceID' => null, 'maininvoiceID' => null, 'amount' => 0, 'due' => 0, 'paidTotal' => 0];
                if (isset($invoiceByStudentFeetype[$key])) {
                    $cell = $invoiceByStudentFeetype[$key];
                }
    ...
    public function saveBulkPayment()
    {
        ...
        if (empty($payment_method) || empty($payments)) {
            $retArray['message'] = 'Invalid request';
            echo json_encode($retArray);
            exit;
        }

        $payments = is_array($payments) ? $payments : json_decode($payments, true);
        $paymentByMainInvoice = [];

        foreach ($payments as $p) {
            $maininvoiceID = isset($p['maininvoiceID']) ? (int)$p['maininvoiceID'] : 0;
            $invoiceID = isset($p['invoiceID']) ? (int)$p['invoiceID'] : 0;
            $amount = isset($p['amount']) ? (float)$p['amount'] : 0;

            if ($maininvoiceID && $invoiceID && $amount > 0) {
                if (!isset($paymentByMainInvoice[$maininvoiceID])) {
                    $paymentByMainInvoice[$maininvoiceID] = [];
                }
                $paymentByMainInvoice[$maininvoiceID][$invoiceID] = $amount;
            }
        }

        $saved = 0;
        $methodSlug = strtolower(trim((string) $payment_method));
        if ($methodSlug !== 'cash' && $methodSlug !== 'cheque') {
            $methodSlug = 'cash';
        }

        foreach ($paymentByMainInvoice as $maininvoiceID => $invoiceAmounts) {
            $invoices = $this->invoice_m->get_order_by_invoice([
                'maininvoiceID' => $maininvoiceID,
                'deleted_at' => 1
            ]);
            ...
            $hasPayment = false;
            foreach ($invoices as $inv) {
                $paid = isset($invoiceAmounts[$inv->invoiceID]) ? $invoiceAmounts[$inv->invoiceID] : 0;
                $paymentData['paidamount_' . $inv->invoiceID] = $paid > 0 ? $paid : '';
                $paymentData['weaver_' . $inv->invoiceID] = '';
                $paymentData['fine_' . $inv->invoiceID] = '';
                if ($paid > 0) {
                    $hasPayment = true;
                }
            }
            ...
            if ($hasPayment) {
                $transaction_id = strtoupper($methodSlug) . random19() . date('ymdhis');
                $paymentService = new \PaymentService($transaction_id);
                $result = $paymentService->add_transaction([
        ...
        $retArray['status'] = TRUE;
        $retArray['message'] = $saved . ' ' . $this->lang->line('invoices_created');
        $this->session->set_flashdata('success', $retArray['message']);
        echo json_encode($retArray);
        exit;
    }
