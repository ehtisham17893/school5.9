...
class Admin_Controller extends MY_Controller
{
    ...
    */
    ...
    private string $_backendTheme;
    private string $_backendThemePath;
    ...
    public function __construct()
    {
        ...
        $this->_backendTheme     = strtolower((string) $siteInfo->backend_theme);
        $this->_backendThemePath = 'assets/inilabs/themes/' . strtolower((string) $siteInfo->backend_theme);
    ...
    private function _frontendManager($siteInfo)
    {
        ...
        if (in_array(uri_string(), $exceptionUris) == false && $this->signin_m->loggedin() == false) {
    ...
    {
        $permission = $action == 'index' || $action == false ? $module : $module . '_' . $action;
            ...
            if (isset($userdata['loginuserID']) && !isset($userdata['get_permission'])) {
                ...
                if (customCompute($features)) {
                    foreach ($features as $featureKey => $feature) {
                        $permissionArray['master_permission_set'][trim((string) $feature->name)] = $feature->active;
                    }
        ...
        } elseif (isset($userdata['loginuserID']) && !isset($userdata['get_permission'])) {
        ...
        $dbMenus               = $this->menuTree(json_decode(json_encode(pluck(
            $this->menu_m->get_order_by_menu(['status' => 1]),
            'obj',
            'menuID'
        )), true), $sessionPermission);
        ...
        if ((isset($sessionPermission[$permission]) && $sessionPermission[$permission] == "no")) {
            if ($permission == 'dashboard' && $sessionPermission[$permission] == "no") {
                if (in_array('yes', $sessionPermission) && $sessionPermission["dashboard"] == 'no') {
    ...
    public function usercreatemail($email = NULL, $username = NULL, $password = NULL)
    {
        ...
        if (customCompute($emailSetting) && $emailSetting->email_engine == 'smtp') {
            if ($emailSetting->smtp_security) {
                $config = [
                    'protocol'    => 'smtp',
                    'smtp_host'   => $emailSetting->smtp_server,
                    'smtp_port'   => (int) $emailSetting->smtp_port,
    ...
    public function reportPDF($stylesheet = NULL, $data = NULL, $viewpath = NULL, $mode = 'view', $pagesize = 'a4', $pagetype = 'portrait')
    {
        ...
        if (!empty($stylesheet)) {
            ...
            $stylesheet = file_get_contents(base_url('assets/pdf/' . $designType . '/' . $stylesheet), false, $context);
            return $this->mhtml2pdf->create($this->data['panel_title'],$stylesheet,$mode);
        } else {
            return $this->mhtml2pdf->create($this->data['panel_title']);
        }
    }
    ...
    public function reportSendToMail($stylesheet = NULL, $data = NULL, $viewpath = NULL, $email = NULL, $subject = NULL, $message = NULL, $pagesize = 'a4', $pagetype = 'portrait')
    {
        ...
        if (customCompute($emailsetting)) {
            if ($path = @$this->mhtml2pdf->create($this->data['panel_title'], $stylesheet,'save')) {
    ...
    public function getHolidays()
    {
        ...
        if (customCompute($holidays)) {
            foreach ($holidays as $holiday) {
                $from_date = strtotime((string) $holiday->fdate);
                $to_date   = strtotime((string) $holiday->tdate);
                $oneday    = 60 * 60 * 24;
                for ($i = $from_date; $i <= $to_date; $i += $oneday) {
        ...
        return customCompute($uniqueHolidays) ? implode('","', $uniqueHolidays) : '';
    }
    ...
    {
        $date_from = strtotime((string) $this->data['schoolyearobj']->startingdate);
        $date_to = strtotime((string) $this->data['schoolyearobj']->endingdate);
        $oneDay = 60 * 60 * 24;
        ...
        $weekendArrays = explode(',', (string) $weekendDay);
    ...
    {
        $date_from = strtotime((string) $this->data['schoolyearsessionobj']->startingdate);
        $date_to   = strtotime((string) $this->data['schoolyearsessionobj']->endingdate);
        $oneDay    = 60 * 60 * 24;
