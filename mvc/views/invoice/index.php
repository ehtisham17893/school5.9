                ...
                <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                    <?php if(permissionChecker('invoice_add')) { ?>
                        <h5 class="page-header">
                            ...
                            &nbsp;
                            <a href="<?php echo base_url('invoice/add_heads_to_invoices') ?>">
                                <i class="fa fa-plus"></i>
                                Add heads to Invoices
                            </a>

                            <div style="width: 15%;float: right"><?php
                            ...
                            <button id="inv_box" class="btn btn-success btn-sm mrg" style="float: right"><i class="fa fa-check-square-o"></i> Print Invoices</button>
                        </h5>
                    <?php } ?>
                <?php } ?>

                <div id="hide-table">
                    <?php if(count($maininvoices) >= 100)
                            {
                                $length = 50;
                            }else{
                                $length = count($maininvoices);
                            }
                    ?>
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer" data-page-length='<?php echo $length; ?>'>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php if(customCompute($maininvoices)) {$i = 1; foreach($maininvoices as $maininvoice) { ?>
                                <tr>
                                    <td><input type="checkbox" name="inv_box" value="<?php echo $maininvoice->maininvoiceID; ?>" id="inv_check_<?php echo $maininvoice->maininvoiceID; ?>" class="invoice_box"/></td>
                                    ...
                                    <td data-title="<?=$this->lang->line('invoice_student')?>">
                                     ...
                                     <td data-title="<?=$this->lang->line('invoice_classesID')?>">
                                        <?php echo $maininvoice->srclasses . " (" .$maininvoice->srsection. ")"; ?>
                                    </td>
                                    ...
                                    <td data-title="<?=$this->lang->line('invoice_balance')?>">
                                        <?php 
                                            if(isset($grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID])) { 
                                                ...
                                                } elseif (isset($grandtotalandpayment['totalweaver'][$maininvoice->maininvoiceID])) {
                                    ...
                                    <td data-title="<?=$this->lang->line('invoice_date')?>">
                                        <?php echo date("d M Y", strtotime((string) $maininvoice->maininvoicedate)) ; ?>
                                    </td>
                                    ...
                                    <td data-title="<?=$this->lang->line('action')?>">
                                        ...
                                        <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                                            <?php if($maininvoice->maininvoicestatus != 2) { echo btn_edit('invoice/edit/'.$maininvoice->maininvoiceID, $this->lang->line('edit')); } ?>
                                        ...
                                        <?php } ?>
                                        <?php if(permissionChecker('invoice_view') && $maininvoice->maininvoicestatus != 2) { echo btn_invoice('invoice/payment/'.$maininvoice->maininvoiceID, $this->lang->line('payment'));} ?>
                                        <?php 
...
<script type="text/javascript">
    ...
    $('#classesID').change(function(event) {
        ...
        /*else if(classesID === '1') {
            window.location.href='/school/invoice/';
        }*/
        else {
            window.location.href='/school5.9/invoice/index1/'+encodeURI(classesID);
        }
    });
    ...
    $('#inv_box').click(function() {
        var inv_boxes = [];
        $.each($("input[name='inv_box']:checked"), function(){
            inv_boxes.push($(this).val());
        });
        if(inv_boxes.length > 0) {
            window.location.href='/school5.9/invoice/bulkInvoicing/'+encodeURI(inv_boxes.join("-"));
        }else{
            alert("Please select any invoice to print!");
        }
