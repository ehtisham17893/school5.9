                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
                                ...
                                <th class="col-sm-4"><?= $this->lang->line('take_exam_name') ?></th>
                                <th class="col-sm-2"><?= $this->lang->line('take_exam_examstatus') ?></th>
                                <th class="col-sm-2"><?= $this->lang->line('take_exam_duration') ?></th>
                                <th class="col-sm-4"><?= $this->lang->line('take_exam_date') ?></th>
                                <th class="col-sm-2"><?= $this->lang->line('action') ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (customCompute($onlineExams)) {
                                $i = 0;
                                foreach ($onlineExams as $onlineExam) {
                                    if ($usertypeID == '3' && ((($student->srclassesID == $onlineExam->classID) || ($onlineExam->classID == '0')) && (($student->srsectionID == $onlineExam->sectionID) || ($onlineExam->sectionID == '0')) && (($student->srstudentgroupID == $onlineExam->studentGroupID) || ($onlineExam->studentGroupID == '0')) && (($onlineExam->subjectID == '0') || (in_array($onlineExam->subjectID, $userSubjectPluck))))) {
                                        $i++;
                                        $currentdate = 0;
                                        ...
                                        if ($lStatusTaken && $onlineExam->examStatus == 2) {
                                        ...
                                        ?>
                                                ...
                                                <td data-title="
                                        <?= $this->lang->line('take_exam_name') ?>
                                        ">
                                                    <?php 
                                        if (strlen((string) $onlineExam->name) > 50) {
                                            echo strip_tags(substr((string) $onlineExam->name, 0, 50) . "...");
                                        } else {
                                            echo strip_tags(substr((string) $onlineExam->name, 0, 50));
                                        }
                                        ?> - 
                                        <?= $examLabel ?>
                                                ...
                                                <td data-title="
                                        <?= $this->lang->line('take_exam_examstatus') ?>
                                        ">
                                                    <?php 
                                        ...
                                        ?>
                                                ...
                                                <td data-title="
                                        <?= $this->lang->line('take_exam_duration') ?>
                                        ">
                                                    <?php 
                                        echo $onlineExam->duration;
                                        ?>
                                                </td>
                                                <td data-title="
                                        <?= $this->lang->line('take_exam_duration') ?>
                                        ">
                                                    <?php 
                                        if (isset($onlineExam->startDateTime) && isset($onlineExam->endDateTime)) {
                                            echo date("d/M/Y", strtotime((string) $onlineExam->startDateTime)) . ' - ' . date('d/M/Y', strtotime((string) $onlineExam->endDateTime));
                                        }
                                        ?>
                                                </td>
                                                <?php 
                                        //if(permissionChecker('take_exam_edit') || permissionChecker('take_exam_delete') || permissionChecker('take_exam_view')) {
                                        ?>

                                                <td data-title="
                                        <?= $this->lang->line('action') ?>
                                        ">
                                        <button class="btn btn-primary btn-sm" onclick="newPopup('
                                        <?= base_url('take_exam/instruction/' . $onlineExam->onlineExamID) ?>
                                        ')">
