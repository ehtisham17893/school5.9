        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <?php if(customCompute($students) > 0 ) { ?>
                    <?php if(permissionChecker('student_edit') && ((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)))) { ?>
                        <div class="row" style="margin-bottom: 15px;" id="bulkSectionBar">
                            <div class="col-sm-3">
                                <select id="bulkSectionID" class="form-control select2">
                                    <option value="0"><?=$this->lang->line('student_select_section')?></option>
                                    <?php if(customCompute($sections)) {
                                        foreach ($sections as $sectionOption) {
                                            echo '<option value="'.$sectionOption->sectionID.'">'.$sectionOption->section.'</option>';
                                        }
                                    } ?>
                                </select>
                            </div>
                            <div class="col-sm-2">
                                <button type="button" id="changeSectionBtn" class="btn btn-primary">
                                    <i class="fa fa-exchange"></i> <?=$this->lang->line('student_change_section')?>
                                </button>
                            </div>
                        </div>
                    <?php } ?>
                                ...
                                <div id="hide-table">
                                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                                        <thead>
                                            <tr>
                                                <?php if(permissionChecker('student_edit') && ((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)))) { ?>
                                                    <th class="col-sm-1"><input type="checkbox" id="checkAllStudents" title="Select all"></th>
                                                <?php } ?>
                                                ...
                                                <tr>
                                                    <?php if(permissionChecker('student_edit') && ((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)))) { ?>
                                                        <td data-title="Select">
                                                            <input type="checkbox" class="student-select-box" name="student_box" value="<?=$student->srstudentID?>">
                                                        </td>
                                                    <?php } ?>
                                                    ...
                                                    <td data-title="<?=$this->lang->line('student_status')?>">
                                                        <div class="onoffswitch-small" id="<?=$student->srstudentID?>">
                                                            <input type="checkbox" id="myonoffswitch<?=$student->srstudentID?>" class="onoffswitch-small-checkbox" name="paypal_demo" <?php if ($student->active === '1') {
                                        ...
                                        <div id="hide-table">
                                            <table class="table table-striped table-bordered table-hover dataTable no-footer">
                                                <thead>
                                                    <tr>
                                                        <?php if(permissionChecker('student_edit') && ((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)))) { ?>
                                                            <th class="col-sm-1"><input type="checkbox" class="checkAllSectionStudents" title="Select all"></th>
                                                        <?php } ?>
...
<script type="text/javascript">
    ...
    $('#checkAllStudents').on('change', function() {
        $('#all .student-select-box').prop('checked', this.checked);
    });

    $(document).on('change', '.checkAllSectionStudents', function() {
        $(this).closest('table').find('.student-select-box').prop('checked', this.checked);
    });

    $('#changeSectionBtn').on('click', function() {
        var studentIDs = [];
        $('.student-select-box:checked').each(function() {
            var id = $(this).val();
        ...
        var sectionID = $('#bulkSectionID').val();
        var classesID = '<?=(int)$set?>';

        if (studentIDs.length === 0) {
            alert('<?=$this->lang->line('student_select_students')?>');
            return;
        }
        if (!sectionID || sectionID === '0') {
            alert('<?=$this->lang->line('student_section_required')?>');
            return;
        }

        $('#changeSectionBtn').prop('disabled', true);

        $.ajax({
            type: 'POST',
            url: "<?=base_url('student/changeSection')?>",
            data: {
                classesID: classesID,
                sectionID: sectionID,
            ...
            success: function(response) {
                $('#changeSectionBtn').prop('disabled', false);
                if (response.status) {
                    toastr["success"](response.message);
                    window.location.reload();
                } else {
                    toastr["error"](response.message || '<?=$this->lang->line('student_section_update_error')?>');
                }
            },
            error: function() {
                $('#changeSectionBtn').prop('disabled', false);
                toastr["error"]('<?=$this->lang->line('student_section_update_error')?>');
            }
        });
    });
