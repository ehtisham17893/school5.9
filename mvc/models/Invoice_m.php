...
class Invoice_m extends MY_Model {
    ...
    public function get_order_by_invoice1($array=NULL) {
        $query = $this->db->select('invoice.*, (Select paymentamount from payment where payment.invoiceID = invoice.invoiceID order by paymentID desc limit 1) as payment_amount')->from('invoice')->where($array)->order_by('invoiceID','ASC');
        $query = $this->db->get();
