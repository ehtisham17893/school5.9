...
class Maininvoice_m extends MY_Model {
    ...
    public function get_maininvoiceID($studentID= NULL, $schoolyearID = NULL) {
        $id = 0;
        ...
        foreach ($query->result() as $row) {
            $id = $row->maininvoiceID;
        }
        return $id;
    }

    public function update_by_ID($data, $id = NULL) {
        $this->db->set('maininvoicestatus',$data);
        $this->db->where('maininvoiceID', $id);
        $this->db->update($this->_table_name);
        return $id;
    }

	public function get_maininvoice_with_studentrelation($schoolyearID = NULL, $classesID = NULL) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
        ...
        if($classesID != NULL) {
            $this->db->where('maininvoice.maininvoiceclassesID', $classesID);
        }
		$this->db->order_by('maininvoice.maininvoiceID', 'desc');
		$query = $this->db->get();
		return $query->result();
